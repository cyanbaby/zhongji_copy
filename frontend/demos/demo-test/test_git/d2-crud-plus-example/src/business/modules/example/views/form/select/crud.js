...
export const crudOptions = (vm) => {
  return {
    ...
    viewOptions: {
      ...
      // http://d2-crud-plus.docmirror.cn/d2-crud-plus/guide/options.html#%E9%85%8D%E7%BD%AE%E4%B8%8Eelement-ui%E9%85%8D%E7%BD%AE%E7%9A%84%E5%AF%B9%E5%BA%94%E5%85%B3%E7%B3%BB
      componentType: 'row'
    },
    formOptions: {
      maxHeight: true,
    ...
    columns: [
      ...
      },
      {
        ...
        dict: {
          ...
          //     }
          // ]
          // tip 也可以不用url 直接写死data  除了表格列单元格数据映射，对应 搜索栏下拉列表, 新增, 编辑
          // data: [
          //   {
          //     value: '1',
          //     label: '打开开',
        ...
        form: {
          ...
          valueChange: function (key, value) {
            ...
            */
            ...
            vm.$nextTick(() => {
              if (value != null) {
                console.log('-----你选择了1', value, vm.crud?.columnsMap?.status)
              } else {
                console.log('-----你选择了2', value)
              }
            })
            */
            ...
            console.log('componentRef._props.dict.data', componentRef._props.dict.data)
          },
          helper: {
            render (h) {
      ...
      },
      {
        ...
        dict: {
          ...
          // 就是请求上面的 url, 上面的 url 如果请求过会被缓存
          // 改成 /dicts/OpenStatusEnum1 响应后 可以处理一下数据
          getData (url, dict) { // 覆盖全局获取字典请求配置
            ...
            return request({
              ...
              // console.group('我是从自定义的getData方法中加载的数据字典')
              // console.log(ret)
              // console.groupEnd()
              return ret.data
            })
      ...
      },
      {
        ...
        form: {
          component: {
            props: {
              dict: { // 此处配置不影响列展示的效率
                clone: true, // TODO 获取成功后clone一份，不影响全局缓存
      ...
      },
      {
        ...
        component: { props: { color: 'auto' } } //  自动染色 指的是表格列
      },
      {
        title: '级联',
        key: 'cascader',
