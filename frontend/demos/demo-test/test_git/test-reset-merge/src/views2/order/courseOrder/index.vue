    ...
    <d2-crud-x ref="d2Crud"
      ...
      <div slot="header">
        ...
        <div class="peculiar"
          ...
          <el-button type="primary"
                     ...
                     @click="batchClick">批量开单</el-button>
        </div>
      ...
      <template slot="subOrderListSlot"
        ...
        <div>
          ...
          }}<span style="color: #49af4f">【{{
              status.filter((res) => res.dictDataValue == scope.row.status)[0]
                .dictDataName
            }}】</span>
        </div>
      ...
      <template slot="deliverObjSlot"
        ...
        <div>
          {{
            scope.row.deliverObj.mobile == "null"
              ? ""
              : scope.row.deliverObj.mobile
          }}
        </div>
      </template>
      <template slot="paymentToolTypeSlot"
        ...
        <div v-if="scope.row.paymentInfo">
          {{
            scope.row.paymentInfo.paymentToolType == "null"
              ? ""
              : scope.row.paymentInfo.paymentToolType
          }}
        </div>
      ...
      <template slot="gmtCreateSlot"
        ...
        <div v-if="scope.row.paymentInfo">
          {{
            scope.row.paymentInfo.gmtCreate == "null"
              ? ""
              : scope.row.paymentInfo.gmtCreate
          }}
        </div>
      </template>
    </d2-crud-x>
...
export default {
  ...
  methods: {
    ...
    selectArea (val) {
      ...
      const code =
        this.form.buyerAddressCode[this.form.buyerAddressCode.length - 1]
    ...
    determineClick () {
      // 后台开通
      this.$api.getbackstageopen(this.tradeNo).then((res) => {
        if (res.code == 1) {
          this.$message({
            message: '后台开通成功',
            type: 'success'
    ...
    exportBtn () {
      ...
      searchData.startTime = searchData.startTime
        ? new Date(searchData.startTime).getTime()
        : ''
      searchData.endTime = searchData.endTime
        ? new Date(searchData.endTime).getTime()
        : ''
      this.$api
        .EXPORT_ORDER({
          ...searchData,
