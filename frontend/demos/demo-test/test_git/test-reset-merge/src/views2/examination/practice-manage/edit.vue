...
<template>
  <d2-container>
    <template slot="header">
      <span class="page-title">{{isAdd}}练习</span>
    </template>
    <div style="overflow:hidden;">
      <el-form class="pull-left" ref="form" :rules="rules" :model="form" label-width="110px">
        <el-form-item label="组卷方式" prop="paperGroupType">
          <el-radio-group v-model="form.paperGroupType" :disabled="disabledType" @change="changeType">
            ...
            <el-radio style="margin-right: 10px" :label="2" v-if="form.paperType !=3" value="手动组卷">手动组卷</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="选择试卷规则" v-if="isShow" prop="paperRuleId">
          <el-select v-model="form.paperRuleId" @change="changeRule" style="width: 100%">
            <el-option v-for="item in ruleData" :key="item.value" :label="item.ruleName" :value="item.paperRuleId">
            </el-option>
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="分类" prop="paperCatId">
          <el-cascader v-model="form.paperCatId" :options="list" style="width: 100%" :props="{
            label: 'catValue',
      ...
      <div class="pull-right" style="position: fixed;right: 50px;z-index: 1;" v-if="form.paperGroupType == 2">
        <el-card class="box-card" style="widthmargin:20px 0;">
          ...
          <p v-for="(item,i) in partList" :key="i"><span
              style="margin-right:50px">{{item.partName}}</span><span>共{{item.subjectList?item.subjectList.length:0}}道题，共{{item.partScore || 0}}分</span>
          </p>
        </el-card>
      </div>
    </div>
        ...
        <div v-for="(item,i) in part.subjectList" :key="i">
          <el-card class="box-card" style="margin:20px 0;">
            <div style="overflow:hidden;color:#A1A1A1;">
              <div class="pull-left">
                <span>难度:
                  {{item.subjectLevel?dynamicTags1.filter(res => res.dictDataValue == item.subjectLevel )[0].dictDataName:""}}</span>
                <span
                  style="margin-left:50px;">类型：{{item.subjectType?dynamicTags2.filter(res => res.dictDataValue == item.subjectType )[0].dictDataName:""}}</span>
              </div>
              <div class="pull-right">
    ...
    <el-dialog :close-on-click-modal=false title="选择试题" append-to-body class="cus-dialog-container"
      :visible.sync="dialog" :destroy-on-close="true" width="90%" top="10px" bottom="10px">
...
export default {
  ...
  data () {
    return {
      numArr: ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二', '十三', '十四', '十五', '十六', '十七', '十八', '十九',
        '二十'
      ],
      dialog: false,
      isShow: true,
    ...
    doSave () {
      if (this.form.paperGroupType == 1) {
        this.$refs.form.validate((valid) => {
          if (valid) {
            this.form.paperCatId = typeof this.form.paperCatId === 'string' ? this.form.paperCatId : this.form
              .paperCatId[this.form.paperCatId.length - 1]
            this.$api.savePaperByRule({
              ...
              this.close({
                ...
                this.$router.push({
                  path: '/examination/practice_manage'
                })
              })
            })
          } else {
