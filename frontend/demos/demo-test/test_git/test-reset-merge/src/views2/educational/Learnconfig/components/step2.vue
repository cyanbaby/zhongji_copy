<template>
    <el-container v-loading="maskLoading">
      <el-main style="height:70vh;">
        ...
        <span class="line"></span>直播目录（总课时：{{currentTotalHours.toFixed(2)}}）
        </div>
        <div class="classifyCard" v-for="(item,i) in studyContentParams" :key="i">
          ...
          <div class="cardBody">
            ...
            <el-link icon="el-icon-plus" :underline="false" type="primary" style="margin-top: 15px;width: 120px;" @click="choseCourse(i)">添加直播</el-link>
          </div>
        </div>
        <!-- 直接添加直播的表格 -->
        <el-table
        ...
        <div style="margin-bottom:10px;" v-if="isOpenFreeSelection==2">
          <el-link icon="el-icon-plus" :underline="false" type="primary" style="margin-top: 15px;width: 120px;" @click="choseCourse('direct')">添加直播</el-link>
        </div>
        <div @click="addClassify" style="width: 126px;margin-top: 15px;">
        <el-button type="primary" :disabled="studyContentInfos.length > 0">添加直播分类</el-button>
        </div>
      </el-main>
          <el-dialog
            top="15px"
...
<script>
import ChoseLive from './choselive/chose-live.vue'
...
export default {
  ...
  data () {
    return {
      ...
      studyContentParams: [
        /*  {
          classificationName: '', // 直播目录名称（学习内容分类名称）
          //requiredNumber: 1, // 直播目录下必选直播的数量
          requiredMinNumber: 0, // 该学习内容分类下必选门数的最小值
          requiredMaxNumber: 0, // 该学习内容分类下必选门数的最大值
          studyContents: [
            {
              courseServiceId: '', // 直播服务商品id【必须】',
              goodsValue: '', // 直播名称【必须】',
  ...
  methods: {
    ...
    // 新增直播
    choseCourse (i) {
      console.log('choseCourse -> this.currentTotalHours', this.currentTotalHours)
      if (i == 'direct') { // 直接添加直播
        this.flag = true
    ...
    choseCourseDefine () {
      ...
      const midArr = arrRemoveRepeat2(this.choseData, arr, 'goodsId', 'liveChannelId')
      let newArr = []
      newArr = midArr.map(e => {
        return {
          courseServiceId: e.courseServiceId,
          goodsValue: e.name,
          sourceLearnHour: e.courseHour,
          goodsId: e.liveChannelId,
          classLearnHour: e.courseHour,
