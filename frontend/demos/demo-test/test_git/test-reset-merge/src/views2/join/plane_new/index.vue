...
<template>
  <d2-container :class="{ 'page-compact': crud.pageOptions.compact }">
    ...
    <d2-crud-x ref="d2Crud"
      ...
      <template slot="publishStatusSlot"
                slot-scope="scope">
        {{ scope.row.publishStatus == 1 ? "未发布" : "已发布" }}
      </template>
      <template slot="activityTimeSlot"
                slot-scope="scope">
        <span v-if="!scope.row.signUpStartTime||!scope.row.signUpEndTime">不限制报名时间</span>
        <span v-else>{{ `${scope.row.signUpStartTime}-${scope.row.signUpEndTime}` }}</span>
      </template>
    </d2-crud-x>
    <el-dialog title="报名表下载"
      ...
      <el-card class="box-card">
        ...
        <el-form :model="form"
                 ref="form">
          <el-form-item label="">
            <el-checkbox-group v-model="form.positionIdList">
...
export default {
  ...
  methods: {
    Statistics ({ row }) {
      ...
      this.$router.push({ path: '/join/statistics', query: { activityId, activityName } })
    },
    personalDownload () {
      this.$router.push({
        path: 'singers/manage',
    ...
    // 提交下载任务
    submitTask () {
      if (this.form.positionIdList.length <= 0) {
        return this.$message({
          message: '请选择岗位',
          type: 'warning'
        })
    ...
    toStudent ({ row }) {
      const { activityId } = row
      this.$router.push({ path: '/join/user1', query: { activityId } })
      // if (row.activityType == 1) {
      //   this.$router.push({ path: '/join/user_new', query: { activityId } })
