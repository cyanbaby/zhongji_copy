 ...
 * @LastEditTime: 2021-07-30 14:59:58
 * @FilePath: \o-front-admin\src\views\base\folder\picture\index.vue
-->
<template>
  <div>
    <TreeContent
 ...
 <crud-search
    ...
    <el-dialog title="上传图片" :visible.sync="imgUploadDialog" width="800px" :close-on-click-modal=false>
      <el-form :model="imgform" ref="form" :rules="rules" label-width="80px">
        <el-form-item label="上传到" prop="dirId">
          <el-cascader
        ...
        <el-form-item label="本地图片">
          <d2-upload
            ref="d2-upload"
            :mergeData="{ dirId, dictTypeCode: 'material', fileType: 'img', uploadCode:'Upload-material-img' }"
...
export default {
  ...
  data () {
    return {
      ...
      form: {},
      rules: {
        dirId: [{ required: true, message: '请选择目标目录', trigger: 'blur' }]
      },
      fileList: [], // 图片列表
  ...
  methods: {
    ...
    // 合并上传
    submitUpload () {
      console.log(this.$refs['d2-upload'].submitUpload())
    },
