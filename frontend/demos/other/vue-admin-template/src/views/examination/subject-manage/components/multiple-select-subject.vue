    ...
    <div v-if="isShowM">
      <div class="subject-header">
        <div class="subject-header-left">
          ...
          <span style="line-height: 40px"> 多选题 </span>
        </div>
      ...
      <div class="subject-answer">
        ...
        <div class="subject-answer-input">
          <el-checkbox-group v-model="submitInfo.givenAnswers">
            <el-checkbox v-for="(item, index) in submitInfo.chooses" :label="item.flag" :key="index">{{item.flag}}</el-checkbox>
          </el-checkbox-group>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    initData (subjectData) {
      ...
      this.$set(this.submitInfo, 'subjectType', 2)
      this.$set(this.submitInfo, 'content', subjectData.content)
      this.$set(this.submitInfo, 'chooses', subjectData.chooses)
      this.$set(this.submitInfo, 'givenAnswers', typeof subjectData.givenAnswers === 'string' ? subjectData.givenAnswers.split(';') : subjectData.givenAnswers)
      ...
      if (this.validate(this.submitInfo)) {
        this.$api
          .ADD_Subject({
            ...
            optionalContent: JSON.stringify(this.submitInfo.chooses.map(res => { return { flag: res.flag, optContent: this.filterP(res.optContent) } })),
            ...
            answer: this.submitInfo.givenAnswers.sort((a, b) => a.localeCompare(b)).join(';'),
          ...
          .then(({ data }) => {
            ...
            if (this.scene) {
              this.$router.push({ path: '/examination/subject_manage' })
            } else {
    ...
    validate (si) {
      ...
      if (si.chooses.length <= 0 || (si.chooses.length && si.chooses.filter(res => !res.optContent).length)) {
  ...
  watch: {
    ...
    'subjectData.givenAnswers' (newVal) {
      console.log('new givenAnswers:', newVal)
      this.$set(this.submitInfo, 'givenAnswers', newVal)
    },
