...
<template>
  ...
  <d2-container :class="{ 'page-compact': crud.pageOptions.compact }">
    ...
    <d2-crud-x ref="d2Crud"
      ...
      <template slot="auditStatusDescSlot"
                slot-scope="scope">
...
<script>
import { crudOptions } from './new-crud'
...
export default {
  ...
  methods: {
    ...
    // getpersonnel () { // 分页查询报考班级人员信息
    //   apis.ApplyForClassPersonnel({
    //     pageNum: 1,
    //     pageSize: 10,
    ...
    exports (num) { // 全部导出
      ...
      if (num == 1) { // 选中导出
        const strs = []
        this.recordIdsList.forEach((item, index) => {
          strs.push(item.recordId)
        })
        api.ExportApplicationClass({
          appId: util.cookies.get('appId'),
          activityId: this.$route.query.activityId,
