<template>
  <el-container style="height:55vh;">
    ...
    <template slot="wareNameSearchSlot" slot-scope="scope">
      ...
      <el-radio-group v-model="wareName"
                      style="margin-top: 10px;"
                      slot="header"
                      @change="change"
                      size="mini">
        <el-radio-button :label="item.stId"
                         v-for="(item,i) in fullList"
                         :key="i">{{item.wareName}}</el-radio-button>
      </el-radio-group>
      <template slot="learningProgressSlot"
                slot-scope="scope">
        <span>{{Number((scope.row.learningProgress*100).toFixed(2))}}%</span>
      </template>
      <template slot="studyTimeSecondSlot"
                slot-scope="scope">
        <span>{{formateTime(scope.row.studyTimeSecond)}}</span>
      </template>
      <template slot="studyEffectiveTimeSecondSlot"
                slot-scope="scope">
