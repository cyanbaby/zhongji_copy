...
{
    let arrayOfNumber = ['x', 'y', 'z']; // 提示 ts(2322)
    arrayOfNumber[3] = 'a'; // 提示 ts(2322)
    arrayOfNumber.push('b'); // 提示 ts(2345)
    let arrayOfString = [1, 2, 3]; // 提示 ts(2322)
...
// any
{
    ...
    anything = 1; // 不会提示错误
    anything = 'x'; // 不会提示错误
    let num = anything; // 不会提示错误
    let str = anything; // 不会提示错误
...
// unknown 以前只能用any
{
    ...
    {
        let result;
        let num = result; // 提示 ts(2322)
        let anything = result; // 不会提示错误
    }
...
// void、undefined、null
{
    ...
    {
        const userInfo = { name: null };
    }
    // 类型守卫（Type Guard)
    {
        const userInfo = { id: 1, name: 'Captain' };
...
// never
{
    ...
    {
        let Unreachable = 1; // ts(2322)
        Unreachable = 'string'; // ts(2322)
        Unreachable = true; // ts(2322)
        let num = Unreachable; // ok
        let str = Unreachable; // ok
        let bool = Unreachable; // ok
    }
    // Dead Code 检测机制
    {
        const str = 'string';
...
// 类型断言（Type Assertion）
{
    // TypeScript 类型检测无法做到绝对智能
    {
        ...
        const greaterThan2 = arrayNumber.find(num => num > 2); // 提示 ts(2322)
    }
    // as 语法
    {
        const arrayNumber = [1, 2, 3, 4];
