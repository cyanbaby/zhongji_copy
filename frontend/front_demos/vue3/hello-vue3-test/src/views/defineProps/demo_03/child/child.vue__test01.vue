<template>
  <div class="middle">
    <h3> Child 子组件</h3>
    测试01 progressBarConfig.value.progress.progressColorKey 能否通过 progress 快捷方式修改，并保持响应<br />
    测试02 progressBarConfig.value.progress = {} 是否会保持响应?
...
const progress = progressBarConfig.value.progress

const handleTest = () => {
  progressBarConfig.value.isShowStatusIcon = !progressBarConfig.value.isShowStatusIcon
}
...
const handleTest02 = () => {
  if (state) {
    progressBarConfig.value.progress = {}
    state = false
  } else {
    progressBarConfig.value.progress = {
      name: '张三',
